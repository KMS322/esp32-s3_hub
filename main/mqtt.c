...
/* 센서 스트리밍은 outbox 폭주 방지를 위해 QoS0 사용 */
#define MQTT_STREAM_QOS 0
...
esp_err_t mqtt_send_data(const char* topic, const char* data)
{
    ...
    for (int attempt = 0; attempt < MQTT_PUBLISH_RETRY; attempt++) {
        msg_id = esp_mqtt_client_publish(mqtt_client, topic, data, data_len, MQTT_STREAM_QOS, 0);
        if (msg_id >= 0) {
            break;
        }
    ...
    if (msg_id < 0) {
        ESP_LOGE(TAG, "MQTT 메시지 발행 실패(재시도 %d회) - outbox 혼잡/메모리 부족", MQTT_PUBLISH_RETRY);
        return ESP_FAIL;
    }
    ...
    ESP_LOGI(TAG, "MQTT 메시지 발행 시작 - 토픽: %s, 데이터 길이: %zu, qos:%d, msg_id: %d",
             topic, data_len, MQTT_STREAM_QOS, msg_id);
    return ESP_OK;
}
